...
class mainCog(commands.Cog): 
	...
	async def bossTime_(self, ctx):
		if ctx.message.channel.id == basicSetting[7]:
			...
			for i in range(fixed_bossNum):
				if fixed_bossTime[i] < datetime.datetime.now() + datetime.timedelta(hours=int(basicSetting[0])+24):
					datelist2.append(fixed_bossTime[i])
	...
	async def Catacombstam_(self, ctx):
		if basicSetting[21] != "1":
			return await ctx.send('```보이스를 사용하지 않도록 설정되어 있습니다.```', tts=False)
		return await PlaySound(ctx.voice_client, './sound/카타콤.mp3')
	...
	async def PartyDungeontam_(self, ctx):
		if basicSetting[21] != "1":
			return await ctx.send('```보이스를 사용하지 않도록 설정되어 있습니다.```', tts=False)
		return await PlaySound(ctx.voice_client, './sound/파던.mp3')
	...
	async def oblivionjen_(self, ctx):
		if basicSetting[21] != "1":
			return await ctx.send('```보이스를 사용하지 않도록 설정되어 있습니다.```', tts=False)
		return await PlaySound(ctx.voice_client, './sound/망각.mp3')
